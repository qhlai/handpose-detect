...
int judge_move()
{
  ...
  {
      pre_arr[0][counter/2-1]=device[0].pitch;//手背的俯仰角
      pre_arr[1][counter/2-1]=device[1].pitch;//手指的俯仰角
      pre_arr[2][counter/2-1]=abs(pre_arr[0][counter/2-1]-pre_arr[1][counter/2-1]);//手背-手指（>0）
  }
  //一共11组数据（count+1组）
  if(counter/2-1>=10)//进行勾画函数图像
  {
