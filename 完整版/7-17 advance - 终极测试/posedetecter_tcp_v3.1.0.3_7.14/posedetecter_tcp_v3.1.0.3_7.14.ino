...
#define LITTLE_LEFT -20
#define LITTLE_RIGHT 20
/////////////////////////////////////////////////////////
...
#define N 3//目前安装的MPU6050的个数
DEVICE device[N];
/*************************************************
      * 
      * 功能：循环队列
**************************************************/
#define MAXQSIZE 11//定义一个循环队列的最大长度

typedef struct
{
  float *base;//初始化的动态分配存储空间
  int front;
  int rear;
}QUEUE;
QUEUE minus_queue;//定义一个队列
float minus_arr[MAXQSIZE];//申请一个浮点型数组储存俯仰角之差

void EnQueue(float e)
{
  if ((minus_queue.rear + 1) % MAXQSIZE == minus_queue.front)
  {
    Serial.println("队列已满！！");
    return;
  }
  minus_queue.base[minus_queue.rear] = e;
  minus_queue.rear = (minus_queue.rear + 1) % MAXQSIZE;
}

void InitQueue()//初始化一个循环队列
{
  int temp_i;
  minus_queue.base=minus_arr;//指针指向申请的数组
  minus_queue.front = 0;//头指针尾指针（抽象的）都等于0
  minus_queue.rear = 0;
  for(i=0;i<10;i++)//写入十个0
  {
    EnQueue(0);
  }
}

void DeQueue(float *e)
{
  if (minus_queue.front == minus_queue.rear)
  {
    Serial.println("空队列！！");
    return;
  }
  *e = minus_queue.base[minus_queue.front];
  minus_queue.front = (minus_queue.front + 1) % MAXQSIZE;
}

int QueueLength()
{
  return (minus_queue.rear - minus_queue.front + MAXQSIZE) % MAXQSIZE;
}
...
{ 
  ...
  int OpisthenarAndThumbState;
 
  float temp_data=0.0;//定义临时从队列中出来的数据
  float temp_minus=0.0;//临时差值定义
  ...
  old_open_status=open_status;//记录状态
    ...
    boolean b1 = digitalRead(SW);//读取目前的开关状态
     ...
     /*for(int temp_ti1=0;temp_ti1<10;temp_ti1++)
      {
        Serial.print(minus_arr[temp_ti1]);Serial.print(",");
      }
      Serial.println(" ");*/
    ...
    //getdata();
      ...
      else
      {
              ...
              if( choose == 0)//choose 实际是0，定义choose=0 为手背
               {
                ...
                Serial.print("roll0: ");Serial.print(device[0].roll);Serial.print(",");
                //Serial.print("angle_dot: ");Serial.print(angle_dot);Serial.print(",");
                Serial.print("pitch0: ");Serial.println(device[0].pitch);
            ...
            else if(choose == 1)
            {
              ...
              Serial.print("roll1: ");Serial.print(device[1].roll);Serial.print(",");
              //Serial.print("angle_dot: ");Serial.print(angle_dot);Serial.print(",");
              Serial.print("pitch1: ");Serial.println(device[1].pitch);
              //counter++;
            }
            else if(choose == 2)
            {
              getdata(2);
              Serial.print("roll2: ");Serial.print(device[2].roll);Serial.print(",");
              //Serial.print("angle_dot: ");Serial.print(angle_dot);Serial.print(",");
              Serial.print("pitch2: ");Serial.println(device[2].pitch);
             OpisthenarAndThumbState =  OpisthenarAndThumb();
             if(OpisthenarAndThumbState==1)
             {
              //检测到大拇指信息
              two_gesture.thumb=1;
              two_gesture.finalnum=4;
             }
            ...
            if(open_status==2&&choose==1)//第二个条件：进行一轮循环读取之后计算
            {
                ...
                temp_minus=fabs(device[0].pitch-device[1].pitch);//计算俯仰角的差值
                DeQueue(&temp_data);//队列中弹出一个
                EnQueue(temp_minus);//队列中压入一个
          ...
          //可视化分析
           ...
           if(choose == 0)//选择
          {
              digitalWrite(C1,HIGH);//有效 
              digitalWrite(C2,LOW);//失效
              digitalWrite(C3,HIGH);//失效
              //Serial.println("NO.1");
              choose = 1;//轮换
          }
          else if(choose == 1)
          {
              digitalWrite(C1,HIGH);//有效
              digitalWrite(C2,HIGH);//失效
              digitalWrite(C3,LOW);//失效
              choose = 2;
          }
          else if(choose == 2)
          {
              digitalWrite(C1,LOW);//有效
              digitalWrite(C2,HIGH);//失效
              digitalWrite(C3,HIGH);//失效
              choose = 0;
          }
...
int judge_move()
{
  ...
  //一共11组数据（count+1组）
    ...
    for(it=0;it<9;it++)//11个数据10个差值
    {
      D_arr[it]=fabs(minus_arr[it+1]-minus_arr[it]);
    }
    for(it=0;it<10;it++)
    {
      if(D_arr[it]>=10&&D_arr[it]<=30)
      {
        tnum++;
      }
      if(tnum>=7)
  ...
  * 修改： 
  * 类型：函数
  * 功能：大拇指与手背上mpu6050分析，判断大拇指与手背的位置
  * void 函数名()
  * 效果：
**************************************************/
/*int  OpisthenarAndThumb()
{
  uint8_t tempstates = 0;
  if(device[0].pitch <= 20 && device[0].pitch > 0)//手背正水平 大拇指不弯曲与弯曲
  {
    device[2].pitch > 0 ?tempstates = 1:tempstates = 2;
    }
  else if(device[0].pitch >= -20 && device[0].pitch < 0)//手背反水平 大拇指不弯曲与弯曲
  {
    device[2].pitch < 0 ?tempstates = 3:tempstates = 4;
    }
    else if(device[0].roll >= 80 && device[0].roll < 100)//手背竖直，四手指向前 大拇指向上，不弯曲与弯曲
  {
    device[2].pitch < 18 ?tempstates = 5:tempstates = 6;
    }
    else if(device[0].roll >= -100 && device[0].roll < -80)//手背竖直，四手指向前 大拇指向下，不弯曲与弯曲
  {
    device[2].pitch > -18 ?tempstates = 7:tempstates = 8;
    }
    return tempstates;
  }*/

  int  OpisthenarAndThumb()//返回值：0为未检测到，1为正常大拇指手势，2为容易错误的大拇指手势
{
  int tempstates = 0;
  float temp_pitch,temp_pitch_thumb,temp_roll,temp_roll_thumb;
  temp_pitch=fabs(device[0].pitch-device[1].pitch);
  temp_pitch_thumb=fabs(device[0].pitch-device[2].pitch);
  temp_roll=fabs(device[0].roll-device[1].roll);
  temp_roll_thumb=fabs(device[0].roll-device[2].roll);
  if(temp_pitch>=150&&temp_pitch_thumb<15)
  {
    tempstates=1;
  }
  else if(temp_roll<=15&&temp_roll_thumb<=15)
  {
    tempstates=2;
  }
  return tempstates;
